...
// Columns actually consumed by the dashboard — avoid SELECT *.
const SESSION_COLS =
  "id,first_seen,last_seen,country,region,city,device_type,os,browser,referrer_source,referrer_url,user_email,user_name,user_avatar";

export const getDashboardAnalytics = createServerFn({ method: "POST" })
  .inputValidator((input: { fingerprint?: string } | undefined) => input ?? {})
  .handler(async ({ data }) => {
    await ensureAdmin();
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");

    // Cheap change-detection: fingerprint = latest session last_seen + count + latest pageview timestamp + count.
    const [{ data: latestSession }, { count: sessionCount }, { data: latestPv }, { count: pvCount }] =
      await Promise.all([
        supabaseAdmin
          ...
          .limit(1)
          .maybeSingle(),
        supabaseAdmin.from("visitor_sessions").select("id", { count: "exact", head: true }),
        supabaseAdmin
          .from("page_views")
          .select("viewed_at")
    ...
    const fingerprint = `${latestSession?.last_seen ?? "0"}|${sessionCount ?? 0}|${latestPv?.viewed_at ?? "0"}|${pvCount ?? 0}`;

    // Client already has the latest snapshot — skip the heavy fetch and payload.
    if (data.fingerprint && data.fingerprint === fingerprint) {
      return { unchanged: true as const, fingerprint };
    }
